...
/* Creditos a Otosaka (https://wa.me/21268430150) */
...
global.authFile = `LuffyDev WhatBot`
...
function purgeSession() {
    let prekey = []
    let directorio = readdirSync("./LuffyDev-FN")
    let filesFolderPreKeys = directorio.filter(file => {
        return file.startsWith('pre-key-')
    })
    prekey = [...prekey, ...filesFolderPreKeys]
    filesFolderPreKeys.forEach(files => {
    unlinkSync(`./LuffyDev-FN/${files}`)
})
...
const directories = ['./LuffyDev-FN/', './jadibts/']
...
let bio = `LuffyDev WhatsBot┃facebook.com/profile.php?id=100090012237351`
